...
from ray_tutorial.evolution_strategies import (
    optimizers,
    policies,
    tabular_logger,
    tf_util,
...
class Worker(object):
    def __init__(self, config, policy_params, env_name, noise, min_task_runtime=0.2):
    ...
    def rollout_and_update_ob_stat(self, timestep_limit, task_ob_stat):
        if (
            ...
            and self.config.calc_obstat_prob != 0
            and self.rs.rand() < self.config.calc_obstat_prob
        ):
            ...
            task_ob_stat.increment(
                obs.sum(axis=0), np.square(obs).sum(axis=0), len(obs)
            )
        else:
    ...
    def do_rollouts(self, params, ob_mean, ob_std, timestep_limit):
        ...
        while len(noise_inds) == 0 or time.time() - task_tstart < self.min_task_runtime:
...
if __name__ == "__main__":
    ...
    workers = [
        Worker(config, policy_params, env_name, noise_array) for _ in range(num_workers)
    ]

    env = gym.make(env_name)
    sess = utils.make_session(single_threaded=False)
    policy = policies.MujocoPolicy(
    ...
    while True:
        ...
        tabular_logger.record_tabular("TimeElapsedThisIter", step_tend - step_tstart)
