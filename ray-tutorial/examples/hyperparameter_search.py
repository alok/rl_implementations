...
# accuracy.
def train_cnn_and_compute_accuracy(
    params, steps, train_images, train_labels, validation_images, validation_labels
):
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as sess:
            ...
            for i in range(1, steps + 1):
                ...
                sess.run(
                    train_step,
                    feed_dict={x: image_batch, y: label_batch, keep_prob: keep},
                )
            # Training is done, so compute the validation accuracy and the current
            # weights and return.
            totalacc = accuracy.eval(
                feed_dict={x: validation_images, y: validation_labels, keep_prob: 1.0}
            )
